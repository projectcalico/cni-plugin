...
func CmdDelK8s(c *calicoclient.Client, ep api.WorkloadEndpointMetadata, args *skel.CmdArgs, conf utils.NetConf, logger *log.Entry) error {
	...
	if err != nil {
		...
		logger.WithField("WorkloadEndpoint", ep.Name).Warning("WorkloadEndpoint does not exist in the datastore, moving forward with the clean up")
...
func releaseIPAddrs(ipNets []cnet.IPNet, calico *calicoclient.Client, logger *log.Entry) error {
