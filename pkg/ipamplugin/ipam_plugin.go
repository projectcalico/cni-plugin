...
func cmdAdd(args *skel.CmdArgs) error {
	...
	if ipamArgs.IP != nil {
		...
		logger.Infof("Calico CNI IPAM assigned addresses IPv4=%v IPv6=%v", assignedV4.IPs, assignedV6.IPs)
		...
		if num4 == 1 && len(assignedV4.IPs) != num4 {
			if num6 == 1 && len(assignedV6.IPs) != 0 {
				logger.Infof("Assigned IPv6 addresses but failed to assign IPv4 addresses. Releasing %d IPv6 addresses", len(assignedV6.IPs))
				// Free the assigned IPv6 addresses when v4 address assignment fails.
				v6IPs := []cnet.IP{}
				for _, v6 := range assignedV6.IPs {
		...
		if num6 == 1 && len(assignedV6.IPs) != num6 {
			if num4 == 1 && len(assignedV4.IPs) != 0 {
				logger.Infof("Assigned IPv4 addresses but failed to assign IPv6 addresses. Releasing %d IPv4 addresses", len(assignedV4.IPs))
				// Free the assigned IPv4 addresses when v4 address assignment fails.
				v4IPs := []cnet.IP{}
				for _, v4 := range assignedV4.IPs {
		...
		if num4 == 1 {
			if len(assignedV4.IPs) != num4 {
				return fmt.Errorf("failed to request %d IPv4 addresses. IPAM allocated only %d", num4, len(assignedV4.IPs))
			}
			ipV4Network := net.IPNet{IP: assignedV4.IPs[0].IP, Mask: assignedV4.IPs[0].Mask}
			r.IPs = append(r.IPs, &current.IPConfig{
				Version: "4",
				Address: ipV4Network,
			})
		...
		if num6 == 1 {
			if len(assignedV6.IPs) != num6 {
				return fmt.Errorf("failed to request %d IPv6 addresses. IPAM allocated only %d", num6, len(assignedV6.IPs))
			}
			ipV6Network := net.IPNet{IP: assignedV6.IPs[0].IP, Mask: assignedV6.IPs[0].Mask}
			r.IPs = append(r.IPs, &current.IPConfig{
				Version: "6",
				Address: ipV6Network,
			})
