...
- name: local
  cluster:
    server: ${KUBERNETES_SERVICE_PROTOCOL:-https}://[${KUBERNETES_SERVICE_HOST_OVERRIDE:-$KUBERNETES_SERVICE_HOST}]:${KUBERNETES_SERVICE_PORT_OVERRIDE:-$KUBERNETES_SERVICE_PORT}
    $TLS_CFG
users:
- name: calico
  user:
