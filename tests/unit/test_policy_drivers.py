...
class NullPolicyDriverTest(unittest.TestCase):
    """
    Test class for NullPolicyDriver class.
    """
    def setUp(self):
        self.network_name = "deny-inbound"
        self.driver = NullPolicyDriver() 
...
class GetPolicyDriverTest(unittest.TestCase): 
    ...
    def test_get_policy_driver_null(self):
        k8s_pod_name = "podname"
        k8s_namespace = "namespace"
        config = {"name": "testnetwork"}
        config["policy"] = {"type": "none"}
        driver = get_policy_driver(k8s_pod_name, k8s_namespace, config)
        assert_true(isinstance(driver, NullPolicyDriver))
